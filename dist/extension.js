...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("sfPackageGen.chooseMetadata", async () => {
        ...
        foo.on("exit", (code, signal) => {
            ...
            try {
                ...
                if (!apiVersion) {
                    ...
                    apiVersion = packageUtils_js_1.FALLBACK_API_VERSION;
                }
                ...
                vscode.window.showWarningMessage("Could not determine API version from org. Using default version " + packageUtils_js_1.FALLBACK_API_VERSION);
                resolve(packageUtils_js_1.FALLBACK_API_VERSION);
            }
        });
    });
}
...
class CodingPanel {
    ...
    _extensionPath;
    _disposables = [];
    reportFolderMap = packageUtils_js_1.REPORT_FOLDER_MAP;
    ...
    static CACHE_FILE_NAME = ".sf-package-generator-cache.json";
    infoMsg = "All metadata selected except ";
    static getCachePath() {
        return path.join(vscode.workspace.workspaceFolders[0].uri.fsPath, ".sf", CodingPanel.CACHE_FILE_NAME);
    }
    static readCache() {
        try {
            ...
            if (age > packageUtils_js_1.CACHE_TTL_MS) {
    ...
    buildPackageMap(selectedNodes) {
        ...
        const mpPackage = (0, packageUtils_js_1.buildPackageMap)(selectedNodes);
    ...
    generatePackageXML(mpPackage, isCopyToClipboard) {
        ...
        const xmlString = (0, packageUtils_js_1.generatePackageXmlString)(mpPackage, this.VERSION_NUM);
        if (!xmlString) {
        ...
        if (isCopyToClipboard) {
            ...
            fs.writeFile(vscode.workspace.workspaceFolders[0].uri.fsPath + "/manifest/package.xml", xmlString, (err) => {
                ...
                vscode.workspace
                    .openTextDocument(vscode.workspace.workspaceFolders[0].uri.fsPath + "/manifest/package.xml")
                    .then((data) => {
        ...
        if (!node.inFolder) {
            vscode.window.withProgress({
                ...
                var p = new Promise((resolve) => {
                    let sfCmd = "sf org list metadata --metadata-type " +
                        mType +
                        " --api-version " +
                        this.VERSION_NUM +
                        " --json";
                    ...
                    foo.on("exit", (code, signal) => {
                        ...
                        try {
                            ...
                            vscode.window.showErrorMessage("Error fetching metadata for " +
                                mType +
                ...
                var p = new Promise((resolve) => {
                    ...
                    foo.on("exit", (code, signal) => {
                        ...
                        try {
                            ...
                            console.error("Error parsing list metadata output for folder type " + folderType + ": " + e);
                            vscode.window.showErrorMessage("Error fetching folder metadata. Make sure your SF CLI is authenticated and up to date.");
                            this._panel.webview.postMessage({
                                command: "listmetadata",
                                results: [],
                                metadataType: mType,
            ...
            var p = new Promise((resolve) => {
                ...
                foo.on("exit", (code, signal) => {
                    ...
                    try {
                        ...
                        console.error("Error parsing list metadata output for folder " + folderNames[index] + ": " + e);
                    }
                    resolve(undefined);
    ...
    cacheChildrenResults(mType, results) {
        try {
    ...
    buildSelectedMetadataMap(metadataTypes) {
        return (0, packageUtils_js_1.buildSelectedMetadataMap)(metadataTypes);
    }
            ...
            var p = new Promise((resolve) => {
                ...
                foo.on("exit", (code, signal) => {
                    ...
                    try {
                        ...
                        metadataObjectsArr = metadataObjectsArr.filter((obj) => {
                            if (packageUtils_js_1.NON_RETRIEVABLE_TYPES.has(obj.xmlName)) {
                        ...
                        vscode.window.showErrorMessage('Error fetching metadata types. Make sure SF CLI (sf) is installed, authenticated, and up to date. Run "sf org display --json" in your terminal to verify.');
                    }
                    resolve(undefined);
                });
    ...
    _getHtmlForWebview() {
        ...
        const scriptPathOnDisk = vscode.Uri.file(path.join(this._extensionPath, "resources", "webview.js"));
