...
(function () {
  ...
  function renderMetaList() {
    ...
    for (const mt of metadataTypes) {
      ...
      row.className =
        "meta-item" + (mt.id === selectedTypeId ? " selected" : "");
  ...
  function renderCompList() {
    ...
    if (visibleCount === 0 && mt.children.length > 0) {
      $compList.innerHTML =
        '<div class="empty-state"><span>No matching components</span></div>';
    } else if (mt.children.length === 0) {
      $compList.innerHTML =
        '<div class="empty-state"><span>No components found</span></div>';
    } else {
  ...
  function updateMetaStatus() {
    const selected = metadataTypes.filter(
      (m) => m.isSelected || m.isIndeterminate,
    ).length;
  ...
  function updateCompStatus(mt) {
    ...
    $compStatus.textContent =
      selCount + " of " + mt.children.length + " components selected";
  }
    ...
    switch (msg.command) {
      case "metadataObjects": {
        ...
        if (msg.fromCache && msg.cacheTimestamp) {
          ...
          $cacheStatus.title =
            "Loaded from cache. Last refreshed: " + cacheDate.toLocaleString();
        } else {
